...
Auction.readable('init', function init(options, fn) {
  ...
  if (error = this.check(options)) return fn(error);

  fn = fn || noop;
  options = options || {};

  this.reset();
  this.times = { created: Date.now() };
  this.id = options.id;
  this.saleId = options.saleId;
  this.saleDate = options.saleDate;
...
Auction.check = function check(data) {
  ...
  if (!data.id) {
    error = new AuctionError('Invalid auction ID.');
  }

  if ('number' !== typeof data.openPrice) {
    error = new AuctionError('Invalid opening price.');
  }

  if ('minPrice' in data && 'number' !== typeof data.minPrice) {
    error = new AuctionError('Invalid minimum price.');
  }
...
Auction.readable('start', function start(data, fn) {
  ...
  if (error) {
    ...
    return fn(new AuctionError(error));
  }
